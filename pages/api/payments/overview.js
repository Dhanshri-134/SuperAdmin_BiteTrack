    ...
    const result = await pgPool.query(`
      ...
      last_payment AS (
        SELECT
          ...
          MAX(mp.created_at) AS last_payment_date
      ...
      SELECT
        ...
        (p.active_users * p.rate) AS amount_per_day,

        COALESCE(lp.last_paid_till, m.created_at::date) AS paid_till,
        ...
        GREATEST(
          (CURRENT_DATE - COALESCE(lp.last_paid_till, m.created_at::date)),
          0
        ) * (p.active_users * p.rate) AS total_due
